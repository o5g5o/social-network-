...
export default function CreatePostModal({ onClose, onPostCreated }) {
  ...
  // Get followers for private post selection using the correct endpoint
  const { data: followers, isLoading: followersLoading } = useQuery({
    queryKey: ["myFollowers"],
    queryFn: async () => {
      const res = await axios.get(
        `http://localhost:8080/my-followers`,
        { withCredentials: true }
      );
      return res.data;
    },
    enabled: true, // Always enable to load followers
  });

  // Reset selected viewers when privacy changes
  useEffect(() => {
    if (privacy !== "private") {
      setSelectedViewers([]);
          ...
          <div className="p-6 flex-1 overflow-y-auto">
            ...
            {privacy === "private" && (
              <div className="mt-4">
                ...
                {showViewerSelection && (
                  <div className="mt-3 border rounded-lg max-h-40 overflow-y-auto">
                    {followersLoading ? (
                      <p className="p-4 text-center text-gray-500">Loading followers...</p>
